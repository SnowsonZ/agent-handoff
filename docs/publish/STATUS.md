...
| ClawHub | `0.1.9` 已发布并转公开（**VirusTotal 未回写，终态待定**） | <https://clawhub.ai/snowsonz/skills/agent-handoff-skill/security-audit> | 12 个上传路径经 workflow 逐路径核对齐全，本次 5 个步骤全绿（含"转公开确认"，未走 warning 分支）。2026-09-02 slug 由 `agent-handoff` 改名为 `agent-handoff-skill`（`clawhub skill rename`，8 个历史版本与审计记录原样保留，旧 slug 重定向）；`v0.1.9` 是内容与 0.1.8 相同的验证版本，专门确认改名后发布链在新 slug 下依然完整可用，验证已通过。审计终态仍待 VirusTotal 回写、主审计在其之后重新合成——0.1.3 即曾先显示 Pass、VT 回写后翻为 Review |
